...
import { Activity, Github, Home } from "lucide-react";
    ...
    <>
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-60 flex-col border-r border-[#1c2035] bg-[#070a14]/85 backdrop-blur-xl md:flex">
        <div className="px-5 pb-4 pt-6">
          ...
          <Link
            href="/"
            className="mt-2.5 inline-flex items-center gap-1.5 rounded-md px-1 py-0.5 text-xs text-[#4a5278] transition-colors hover:text-[#8892b0] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4f6ef7]/60"
          >
        ...
        <nav className="flex-1 px-3" aria-label="Primary navigation">
          {NAV_GROUPS.map((group, groupIdx) => (
            <div key={group.id}>
              {groupIdx > 0 && (
                <div className="mx-2 my-2 border-t border-[#1c2035]/70" aria-hidden />
              )}
        ...
        <div className="px-5 py-5 space-y-2">
          <a
            ...
            aria-label="View RevLoop source on GitHub (opens in new tab)"
            className="flex items-center gap-2 rounded-lg px-1 py-0.5 text-xs text-[#4a5278] transition-colors hover:text-[#8892b0] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4f6ef7]/60"
          >
      ...
      <nav
        className="fixed inset-x-3 bottom-3 z-40 grid grid-cols-8 rounded-2xl border border-[#1c2035] bg-[#070a14]/95 p-1.5 shadow-[0_0_32px_rgba(0,0,0,0.45)] backdrop-blur-xl md:hidden"
        aria-label="Mobile navigation"
      >
        <Link
          href="/"
          aria-label="Back to Home"
          className="flex min-h-12 items-center justify-center rounded-xl text-[#4a5278] transition-colors hover:bg-[#0f1322] hover:text-[#eef2ff] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4f6ef7]/60"
        >
